...
import com.example.homework.bean.GoodsInfoGoods;
...
public class GoodsDBHelper extends SQLiteOpenHelper {
    ...
    // 往该表添加一条记录
    public long insert(GoodsInfoGoods info) {
        //These codes are from Peter Pang.
        ArrayList<GoodsInfoGoods> infoArray = new ArrayList<GoodsInfoGoods>();
        infoArray.add(info);
        return insert(infoArray);
    }

    // 往该表添加多条记录
    public long insert(ArrayList<GoodsInfoGoods> infoArray) {
        long result = -1;
        for (GoodsInfoGoods info : infoArray) {
    ...
    // 根据条件更新指定的表记录
    public int update(GoodsInfoGoods info, String condition) {
        ContentValues cv = new ContentValues();
        cv.put("name", info.name);
        cv.put("desc", info.desc);
        cv.put("price", info.price);
    ...
    public int update(GoodsInfoGoods info) {
        // 执行更新记录动作，该语句返回记录更新的数目
        return update(info, "rowid=" + info.rowid);
    }

    // 根据指定条件查询记录，并返回结果数据队列
    public ArrayList<GoodsInfoGoods> query(String condition) {
    ...
    // 根据行号查询指定记录
    public GoodsInfoGoods queryById(long rowid) {
        GoodsInfoGoods info = null;
        ArrayList<GoodsInfoGoods> infoArray = query(String.format("rowid='%d'", rowid));
        if (infoArray.size() > 0) {
