...
import com.example.homework.adapter.GoodsAdapter.addCartListener;
...
public class ShoppingChannelActivity extends AppCompatActivity implements View.OnClickListener, addCartListener{
    ...
    private void showGoods() {
        // 查询商品数据库中的所有商品记录
        ArrayList<GoodsInfoGoods> goodsArray = mGoodsHelper.query("1=1");
        // 构建商场中商品网格的适配器对象
    ...
    private void downloadGoods() {
        ...
        if (mFirst.equals("true")) { // 如果是首次打开
            ArrayList<GoodsInfoGoods> goodsList = GoodsInfoGoods.getDefaultList();
            for (int i = 0; i < goodsList.size(); i++) {
                GoodsInfoGoods info = goodsList.get(i);
                // 往商品数据库插入一条该商品的记录
            ...
            for (int i = 0; i < goodsArray.size(); i++) {
                GoodsInfoGoods info = goodsArray.get(i);
                // 从指定路径读取图片文件的位图数据
